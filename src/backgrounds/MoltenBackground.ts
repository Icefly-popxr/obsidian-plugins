...
/**
 * MoltenBackground — 熔岩金属 WebGL 动态背景
 * 从 React Bits 的 MoltenMetal 移植为原生 TS 类（无 React 依赖），
 * 供 Obsidian 插件视图使用。Shader 与参数逻辑 verbatim 保留。
 */

export interface MoltenParams {
  color1?: string;       // '#5227FF'
  color2?: string;       // '#FF9FFC'
...
const DEFAULT_PARAMS: Required<MoltenParams> = {
  color1: "#5227FF",
  color2: "#FF9FFC",
  color3: "#FFFFFF",
  speed: 0.35,
...
function colorModeToFloat(mode: MoltenParams["colorMode"]): number {
...
export class MoltenBackground {
  ...
  /** 挂载到容器并启动渲染 */
  mount(container: HTMLElement, params: MoltenParams = {}): void {
    this.unmount();
  ...
  /** 更新参数（热更新，无需重建） */
  setParams(params: MoltenParams = {}): void {
